...
function thesis_avatar($display) {
	if (get_option('show_avatars')) {
		...
		if (empty($author_url) || $author_url == 'http://')
			...
			$output = $output . '<a href="' . $author_url . '" rel="external nofollow">' . $avatar . '</a>';
...
/**
 * function thesis_comment_navigation()
 *
 * Display comment navigation links
 *
 * @since 1.5
 */
function thesis_comment_navigation($position = 1) {
	...
	if (get_option('page_comments')) {
		...
		if ($total_pages > 1) {
			echo '				<div id="comment_nav_' . $position . '" class="prev_next">' . "\n";
	
			if ($current_page == $total_pages) {
				echo '					<p class="previous">';	
				previous_comments_link('&larr; ' . __('Previous Comments', 'thesis'));
				echo "</p>\n";
			}
			elseif ($current_page == 1) {
				echo '					<p class="next">';
				next_comments_link(__('Next Comments', 'thesis') . ' &rarr;');
				echo "</p>\n";
			}
