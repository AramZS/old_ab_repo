	...
	function widths() {
		...
		if ($this->layout['columns'] == 2 || $this->layout['columns'] == 3) {
			...
			if ($this->layout['order'] == 'normal') { // Content on the left
				$widths['post_box_margin_left'] = $widths['comment_padding'] = $widths['prev_next_padding_left'] = $widths['prev_next_padding_right'] = $horizontal_spacing['half'];
				...
				if ($this->layout['columns'] == 2)
					$widths['post_box_margin_right'] = $widths['comment_margin_right'] = $horizontal_spacing['single'] + 1;
				else
					$widths['post_box_margin_right'] = $widths['comment_margin_right'] = $horizontal_spacing['single'];
			}
...
.login_alert { ' . $special_comment_css . ' }';
...
#image_box, #video_box, #custom_box { padding: ' . round((($widths['mm_box_padding'] / 2) / $this->base['num']), 1) . 'em; }
	...
	#image_box img.one_by_two { width: ' . round(($widths['multimedia_image'] / $this->base['num']), 1) . 'em; height: ' . round(($height_1x2 / $this->base['num']), 1) . 'em; }';
		...
		if ($this->multimedia_box['status'])
			$special_sidebar_css .= '	.sidebar ul.sidebar_list { padding-top: 0; }';
...
#header #tagline { font-size: ' . round(($this->fonts['sizes']['content'] / $this->base['num']), 1) . 'em; line-height: ' . round((($this->fonts['sizes']['content'] + 6) / $this->fonts['sizes']['content']), 3) . 'em; ' . $font_families['byline'] . '}
...
.comments_closed { margin: ' . $spacing['double'] . 'em 0 ' . $spacing['line_height'] . 'em 1.1em; }
